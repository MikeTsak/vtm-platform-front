...
const srcPath = 'x:/Projects/Vampire Platform/front/src/components/MeritsBackgroundsSection.jsx';
let content = fs.readFileSync(srcPath, 'utf8');

// Find the start of the state we want to replace
const stateStart = content.indexOf('  const [meritQ, setMeritQ] = useState(\'\');');
if (stateStart === -1) {
    console.error("Could not find meritQ");
    process.exit(1);
}

const beforeState = content.slice(0, stateStart);

const newStateAndRender = `
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', minHeight: '600px' }}>
        ...
        {/* Explorer Panels */}
        <div style={{ display: 'flex', flex: '2', gap: '24px', flexWrap: 'wrap' }}>
        ...
        {/* Detail Pane */}
        <div style={{ flex: '1', minWidth: '300px', display: 'flex', flexDirection: 'column', backgroundColor: 'rgba(28, 27, 27, 0.6)', backdropFilter: 'blur(12px)', border: '1px solid var(--border-color)', maxHeight: '600px', position: 'relative' }}>
          {selectedAdvantage ? (() => {
        ...
        <div style={{ backgroundColor: 'rgba(28, 27, 27, 0.6)', backdropFilter: 'blur(12px)', border: '1px solid var(--border-color)', padding: '24px', marginTop: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
            {[...meritsList, ...backgroundsList].map((m, i) => {
              ...
              return (
                <div key={m.id + i} style={{ border: '1px solid var(--border-color)', padding: '16px', display: 'flex', gap: '16px', backgroundColor: 'var(--surface-container-low)', position: 'relative' }}>
                  <div style={{ width: '64px', height: '64px', backgroundColor: 'var(--surface-color)', border: '1px solid var(--border-color)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span className="material-symbols-outlined" style={{ color: 'var(--text-muted)', fontSize: '32px' }}>star</span>
                  </div>
                  <div style={{ flexGrow: 1 }}>
                    ...
                    <input 
                      ...
                      defaultValue={m.notes ? (typeof m.notes === 'string' ? m.notes : JSON.stringify(m.notes)) : ''}
                      readOnly
                      style={{ width: '100%', backgroundColor: 'transparent', border: 'none', borderBottom: '1px solid var(--border-color)', color: 'var(--text-color)', marginTop: '8px', fontSize: '14px', outline: 'none' }}
                    />
                  </div>
                </div>
              );
            ...
            {flawsList.map((f, i) => {
              ...
              return (
                <div key={f.id + i} style={{ border: '1px solid var(--border-color)', borderLeft: '4px solid var(--error)', padding: '16px', display: 'flex', gap: '16px', backgroundColor: 'var(--surface-container-low)', position: 'relative' }}>
                  <div style={{ width: '64px', height: '64px', backgroundColor: 'var(--surface-color)', border: '1px solid var(--border-color)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span className="material-symbols-outlined" style={{ color: 'var(--error)', fontSize: '32px' }}>visibility_off</span>
                  </div>
                  <div style={{ flexGrow: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
...
const newFileContent = beforeState + newStateAndRender + "\nexport default MeritsBackgroundsSection;\n";

fs.writeFileSync(path.join(__dirname, 'MeritsBackgroundsSection.jsx.new'), newFileContent);
console.log("Generated new component file.");
