...
exports.getCoterie = getCoterie;
exports.summarizeCoterie = summarizeCoterie;
exports.ALL_COTERIE_NAMES = exports.COTERIES = exports.COTERIE_DOC = void 0;
...
var COTERIE_DOC = {
  overview: ["Players may choose one of the example coterie types below or invent their own.", "Build what the troupe desires first, rules second. Swap prerequisites freely if the table prefers.", "Coterie types can change during play; adjust advantages to reflect the story."],
  pool_rules: ["Start with a Coterie pool equal to members × points_per_member (default 1; ST may allow 2).", "Pay listed costs from the coterie pool. If it’s not enough, collect from characters.", "Increase any listed value with remaining pool; use 'Possible Extras' as sinks for leftovers."],
  bonus_rules: ["At ST discretion, if all prerequisites of a type are met, grant +1 dot to one Domain Trait (not above 5).", "If a coterie later changes and would exceed 5, refund a dot or XP accordingly."],
  bonus_eligible: {
  ...
  core_greek: ["Ελάχιστοι 3 παίκτες φτιάχνουν Κοτέρι και ΠΡΕΠΕΙ να επιλέξουν Domain. Χωρίς domain, δεν υπάρχει Κοτέρι.", "Βασικά Domain Traits: Chasse (feeding), Lien (προσαρμογή), Portillon (ασφάλεια).", "Κάθε μέλος δίνει 1 (ή 2) Coterie Point στη δημιουργία. Οι πόντοι μοιράζονται σε Chasse/Lien/Portillon ή σε coterie Backgrounds.", "Τα ατομικά XP ΔΕΝ ξοδεύονται για το Κοτέρι. Υπάρχει ξεχωριστό Coterie XP pool (reward καλού ομαδικού play από τον ST).", "Τα Backgrounds του Κοτεριού ανήκουν στο Κοτέρι. Η χρήση τους «χρεώνει» όλους. Κάθε αλλαγή απαιτεί ΟΜΟΦΩΝΙΑ."]
...
exports.COTERIE_DOC = COTERIE_DOC;
var COTERIES = {
  "Blood Cult": {
    domain: {
      lien: dots(1),
  ...
  "Regency": {
    ...
    // or ••• for Anarch
    extras: ["Up to 10 dots among Haven/Herd/Influence/Resources/Retainers/Status: City Secrets/Coterie Advantages", "Equal dots of Flaws among Adversaries/Compromised Haven/Despised/Enemies/Stalkers/Coterie Flaws"],
    preferred_resonances: ["Phlegmatic"],
  ...
  "Saboteur": {
    domain: null,
    // may later add Domain if embedded
    required: {
      Contacts: dots(2),
      Influence: dots(1),
      Mawla: dots(2),
  ...
  "Sbirri": {
    domain: {
      /* mimics target coterie; spend one less dot overall */
    },
    required: {
      Mawla: dots(2),
      Mask: dots(1)
    },
    extras: ["Adversaries (target city Primogen)", "Status: City Secrets", "Advantages from the cover type", "Dark Secret (••)"],
    preferred_resonances: ["Melancholy"],
  ...
  "Primogen's Council": {
    domain: {
      /* Usually none shared; may add +1 dot to Chasse or Portillon */
    },
    required: {
      Haven: dots(3)
    },
...
exports.COTERIES = COTERIES;
var ALL_COTERIE_NAMES = Object.keys(COTERIES);
exports.ALL_COTERIE_NAMES = ALL_COTERIE_NAMES;

function getCoterie(name) {
  return COTERIES[name] || null;
}

function summarizeCoterie(name) {
  var c = getCoterie(name);
  if (!c) return null;
  var dom = c.domain ? ["chasse", "lien", "portillon"].filter(function (k) {
    return c.domain[k];
  }).map(function (k) {
    return "".concat(cap(k), " \u2022").repeat(c.domain[k] || 0);
  }).join(" ") : "None";
  return {
    name: name,
