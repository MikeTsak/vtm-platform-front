      ...
      <div style={{ background: 'var(--glass-bg)', backdropFilter: 'var(--glass-blur)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-lg)', padding: '1.5rem', boxShadow: 'var(--glass-shadow)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '2rem', alignItems: 'start' }}>
          <form onSubmit={handleCreateIdentity} style={{ background: 'var(--glass-inset)', padding: '1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--glass-border)' }}>
            <h4 style={{ margin: '0 0 1rem 0', color: 'var(--accent-purple)' }}>Generate Address Vector</h4>
            <div style={{ marginBottom: '10px' }}><label className={styles.labeledInput}><span>Tag Label descriptor</span><input className={styles.input} placeholder="e.g. Executive Office" value={formDisplay} onChange={e => setFormDisplay(e.target.value)} required /></label></div>
            <div style={{ marginBottom: '15px' }}><label className={styles.labeledInput}><span>Routing Target Address</span><input className={styles.input} placeholder="e.g. contact@city.gov" value={formEmail} onChange={e => setFormEmail(e.target.value)} required /></label></div>
            <button className={styles.btnPrimary} style={{ width: '100%', padding: '0.6rem' }} disabled={isSubmitting}>{isSubmitting ? 'Processing...' : 'Mount Node'}</button>
          </form>

          <div className={styles.tableContainer} style={{ maxHeight: '230px', overflowY: 'auto' }}>
            <table className={styles.table}>
              <thead><tr><th>Descriptor</th><th>Address Node</th><th style={{ width: 50 }}>Directives</th></tr></thead>
              <tbody>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', height: '620px', background: 'var(--glass-bg)', backdropFilter: 'var(--glass-blur)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', boxShadow: 'var(--glass-shadow)' }}>
        ...
        <div style={{ borderRight: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.2)' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {threads.length === 0 && <div style={{ padding: '3rem 1rem', textAlign: 'center', color: 'var(--text-muted)', fontStyle: 'italic' }}>Buffer pipe empty.</div>}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', background: 'var(--glass-inset)' }}>
          {selectedThreadId && activeThread ? (
            <>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                {messages.map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{ alignSelf: isIdentity ? 'flex-end' : 'flex-start', maxWidth: '80%', background: isIdentity ? 'linear-gradient(135deg, rgba(127,90,240,0.2) 0%, rgba(157,124,255,0.05) 100%)' : 'rgba(255,255,255,0.03)', padding: '1rem 1.25rem', borderRadius: 'var(--radius-md)', border: `1px solid ${isIdentity ? 'var(--glass-border-highlight)' : 'var(--glass-border)'}`, boxShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>
                      <div style={{ fontSize: '0.75rem', color: isIdentity ? 'var(--accent-purple)' : 'var(--text-secondary)', fontWeight: 700, marginBottom: '6px', display: 'flex', justifyContent: 'space-between', gap: '2rem' }}>
                        <span>{isIdentity ? activeThread.identity_name : activeThread.user_name}</span>
              ...
              <div style={{ padding: '1.5rem 2rem', borderTop: '1px solid var(--glass-border)', background: 'rgba(0,0,0,0.2)' }}>
                <TextEditor placeholder={`Transmit reply back through address disguise mask [ ${activeThread.identity_name} ]...`} value={reply} onChange={setReply} />
                <div style={{ textAlign: 'right', marginTop: '1rem' }}>
                  <button className={styles.btnPrimary} onClick={handleReply} disabled={!reply.trim()}>Dispatch Transmission Pipeline</button>
                </div>
              </div>
            </>
          ) : (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.4, flexDirection: 'column', gap: '1rem' }}>
              ...
              <h3 style={{ margin: 0, color: 'var(--text-secondary)' }}>Await Selection Intercept Matrix</h3>
            </div>
          )}
        </div>
      </div>
